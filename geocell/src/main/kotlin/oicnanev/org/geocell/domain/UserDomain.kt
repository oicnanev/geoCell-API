...
class UserDomain (
    ...
    fun isSafePassword(password: String): Boolean {
        if (password.length < 8) return false                                       // at least 8 characters long
        if (password.firstOrNull { it.isDigit() } == null) return false             // at least 1 digit
        if (password.filter { it.isLetter() }.firstOrNull { it.isUpperCase() }
                == null) return false                                               // at least 1 uppercase letter
        if (password.filter { it.isLetter() }.firstOrNull { it.isLowerCase() }
                == null) return false                                               // at least 1 lowercase letter
